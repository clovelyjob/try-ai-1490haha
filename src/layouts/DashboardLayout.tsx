...
import {
  Home, FileText, Briefcase, Settings, Mic, Shield, Menu, ArrowUpRight, LogOut, ChevronLeft,
} from 'lucide-react';
...
export default function DashboardLayout() {
  ...
  const mainNav = useMemo(() => {
    ...
    if (isAdmin) items.push({ icon: Shield, label: 'Admin', path: '/dashboard/admin' });
    return items;
  }, [isAdmin, t]);
      ...
      <div className="min-h-screen flex w-full overflow-x-hidden bg-background">
        {isMobile && (
          <Button 
            ...
            className="fixed top-3 left-3 z-50 h-9 w-9 md:hidden bg-card border border-border/40 shadow-clovely-sm"
            onClick={() => setDrawerOpen(true)}
          >
            <Menu className="h-4 w-4" />
          </Button>
